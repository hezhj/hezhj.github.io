...
```
The MLE of $\lambda$ is $\hat\lambda_{MLE}=`r lambda1`$.
...
The MLE of $\lambda$ is $\hat\lambda_{MLE}=`r lambda2`$.

### What's the MLE for Weibull distribution

The Weibull distribution with shape parameter $a>0$ and scale parameter $b>0$ has density given by
$$f(x;a,b) = (a/b) (x/b)^{a-1} \exp(- (x/b)^a)$$
for $x > 0$. The cumulative distribution function is $F(x) = 1 - \exp(- (x/b)^a)$ on $x > 0$.

```{r,fig.align='center'}
plot(seq(0,5,by=0.01),dweibull(seq(0,5,by=0.01), shape = 2),type='l',xlab='x',ylab='density',main='Weibull distribution with a=2,b=1',bty = "l")
```


Suppose $Y_i\stackrel{iid}\sim f(y;a,b)$. How to estimate $a$ and $b$ for the two data sets above? The likelihood  equation cannot be solved algebraically so you may need a numerical procedure.
...
How to estimate the parameters $w_j,\mu_j,\sigma_j^2$?
